...
router.get('/showPictures',function(req,res){
    ...
    User_imageBox.find(boxToFind,function (err, user_box) {
        for(var i=0;i<user_box[0].pictures.length;i++){
            var url="upload/"+user_box[0].username+"/box-small-"+user_box[0].pictures[i];
            var m1=-125*(i%6);
        ...
        res.json({"html_pictures":html_pictures,"title":user_box[0].title});
    });
});
